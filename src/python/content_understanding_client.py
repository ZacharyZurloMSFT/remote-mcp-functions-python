...
class AzureContentUnderstandingClientError(RuntimeError):
    """Raised when the Content Understanding client encounters an error."""
...
class AzureContentUnderstandingClient:
    ...
    def begin_analyze(
        ...
        Args:
            ...
            content_type (str, optional): Explicit MIME type to send with the request.
        ...
        Raises:
            ...
            AzureContentUnderstandingClientError: If the REST request fails.
        """
        ...
        if file_path.exists():
            if file_path.is_dir():
                data = {
                    "inputs": [
                        {
                            "name": "_".join(f.relative_to(file_path).parts),
        ...
        try:
            ...
            raise AzureContentUnderstandingClientError(
                f"Analyze request failed for analyzer '{analyzer_id}': {exc}"
            ) from exc
    ...
    def poll_result(
        ...
        while True:
            ...
            try:
                ...
                raise AzureContentUnderstandingClientError(
                    f"Polling analyze result failed: {exc}"
                ) from exc
            ...
            if status == "succeeded":
                ...
                self._logger.error("Request failed. Reason: %s", payload)
                raise AzureContentUnderstandingClientError("Analyze request failed.")
            else:
                self._logger.info(
